...
### 0.5.1 외부 스킬과의 역할 분담 (`ui-ux-pro-max`)
환경에 `ui-ux-pro-max` 스킬(84 스타일 / 192 팔레트 / 74 폰트 페어링 검색 DB)이 설치되어 있을 수 있다.
유용한 스킬이지만 **작동 원리가 이 에이전트와 정면으로 충돌**하므로 용도를 엄격히 나눈다.

* **충돌 지점:** ui-ux-pro-max는 미리 정의된 카탈로그에서 스타일·팔레트·폰트를 **검색해 고른다.**
  craft-skill의 명제는 그 반대다 — 참조 장르를 웹사이트가 아닌 매체에서 가져오고,
  카탈로그에 없는 방향을 스스로 만든다. 카탈로그 선택은 모델 기본값보다는 낫지만 **여전히 평균값**이다.
  (실측 예: "industrial B2B manufacturer" 질의에 "게이밍·소셜미디어·청년층용 Vibrant & Block-based"
  스타일과 Outfit/Work Sans 조합이 반환됨 — 산업 제조사에 부적합.)

* **이 에이전트 작업(랜딩·브랜드 사이트)에서는:**
  ui-ux-pro-max의 **스타일·컬러 팔레트·폰트 페어링 추천을 채택하지 않는다.**
  아트 디렉션은 섹션 0.5 절차대로 직접 수립한다. 요청받지 않았다면 `--design-system`을 실행하지 않는다.

* **단, 다음은 참조해도 좋다** (평균값 위험이 없고 검증된 규칙):
  접근성 기준(대비 4.5:1, 포커스 링 유지, aria) · 터치 타겟 44×44px · 모션 지속시간 150~300ms ·
  `prefers-reduced-motion` · 모바일 safe area · 폼 UX(라벨 가시성, 필드 근처 에러) · 차트 접근성.

* **ui-ux-pro-max가 더 적합한 작업:** 대시보드 · 관리자 화면 · 앱 UI · 데이터 시각화처럼
  **차별화보다 표준 준수와 예측 가능성이 미덕인 영역.** 그런 요청이면 그 스킬을 쓰도록 안내한다.

**요약: 랜딩·브랜드 = craft 우선(카탈로그 선택 금지). 대시보드·앱 UI = ui-ux-pro-max 우선.
접근성·터치·모션 규칙은 언제나 참조 가능.**
...
### B-2. 재사용 브랜드는 DESIGN.md로 남긴다
여러 페이지·여러 세션에 걸쳐 유지될 브랜드라면, 토큰을 CSS 변수로만 두지 말고
**DESIGN.md**(YAML 토큰 + 근거 산문)로 기록한다. 형식과 예시는 `craft-skill` 섹션 3-D-2 참조.
값만 남기면 다음 작업자가 근거를 몰라 평균값으로 되돌아간다 — **왜 그 값인지를 함께 적는다.**

검증은 린터에 맡긴다(대비 미달은 눈으로 잡기 어렵다):
```bash
npx @google/design.md lint DESIGN.md      # WCAG 대비 · 깨진 토큰 참조 · 구조
npx @google/design.md diff A.md B.md      # 개정 시 회귀 탐지
```
단, 린터는 접근성·일관성만 본다. **"평균값인가"는 판단하지 못하므로** craft 검수는 그대로 수행한다.
