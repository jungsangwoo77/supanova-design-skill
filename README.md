...
5개의 스킬이 각각의 폴더에 `SKILL.md` 파일로 존재합니다.
...
### 5. craft-skill (Supanova Design Craft)
**"AI가 만든 티"를 제거하는 품질 레이어.** 다른 스킬 위에 얹어 씁니다. 표면적 금지(퍼플 그라디언트·Inter 금지)를 넘어, 동질화의 진짜 원인인 **통계적 평균값 출력**을 차단합니다.

- **아트 디렉션 우선** — 코드 전에 컨셉·지배 요소·의도적 제약·참조 장르·감정 목표를 확정
- **제약된 시스템** — 간격/타입 스케일 고정, 그레이스케일 우선, 위계는 명도·웨이트로
- **구성적 긴장** — 대비 3:1 이상, 밀도 교차, 의미 없는 카드 포장 금지
- **에디토리얼 카피** — 브랜드명을 지웠을 때 다른 회사에 못 쓰는 문장만 허용
...
위 4개 프롬프트 디자인을 하나로 통합한 **Claude Code 서브에이전트**가 `.claude/agents/supanova-designer.md`에 포함되어 있습니다. taste(생성) · soft(에이전시 미학) · redesign(업그레이드) · output(완전 출력)의 규칙을 한 에이전트에 반영해, 작업 요청에 따라 CREATE / REDESIGN 모드를 자동으로 선택합니다.

Claude Code에서 이 저장소를 열면 에이전트가 자동으로 인식됩니다.

```
# 서브에이전트 목록 확인
/agents

# 자연어로 위임 (Claude가 supanova-designer에게 라우팅)
"SaaS 랜딩페이지 하나 프리미엄하게 만들어줘"
"이 페이지를 $150k 에이전시 퀄리티로 리디자인해줘"
```
